...
def load_hk(path, so2_cylinder_conc):
    ...
    df = df[
        ["Task", "ZA_SB_MFC_Read", "Cal_SO2_MFC_Read", "Time_s", "Cell_Flow", "Signal_LIF_Counts", "SO2_ppt", "Inlet_ZA_MFC_Read", "SO2_Cal_MFC_Set_P", "Cal_SB_Vl", "Cell_Pressure", "Laser_Power_PT_0", "Thermistor_5", "T_board_NLO", "T_board_REF_cell"]
    ]
...
def identify_cal_amb_groups(
    ...
    cal_timerange_mod = cal_timerange[3:33]
    #cal_timerange_mod = pd.concat(cal_timerange_mod[:])
    amb_timerange_mod = amb_timerange[:45]
...
def calculate_za_cal_points(hk, counts, cal_times, cyl_conc=5030, remove_start=0, remove_end=0):
    ...
    for i, row in enumerate(cal_times[:42].itertuples()):
        mr = (
