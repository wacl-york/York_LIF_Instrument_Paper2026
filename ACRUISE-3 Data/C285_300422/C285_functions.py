...
def reprocess_binary_data_sig(bin_file_path, log_start_datetime, sensitivity=1, bckgrnd=0, data_freq=10, skip_start=0
    ...
    file_shift = pd.read_csv('misalligned_files.txt')
    ...
    for file in filt_file_list:
        ...
        #lin_counts = binary_data_dict['sig_counts']
        binary_data_dict['sig_counts_lin'] = np.log(1 - (binary_data_dict['sig_counts']/max_counts)) * -max_counts
        #plt.plot(binary_data_dict['sig_counts_lin'], color='orange', alpha=0.7)
        ...
        for i in range(len(binary_data_dict['time_ms']) - ((10 * group_avg) + 1)):
            ...
            if binary_data_dict['seed_LD_mode'][i + shift] == 5:
            ...
            if binary_data_dict['seed_LD_mode'][i + shift] == 6:
            ...
            if binary_data_dict['seed_LD_mode'][i + shift] == 6 and binary_data_dict['seed_LD_mode'][i + shift + 1] == 5:
...
def reprocess_binary_data_ref(bin_file_path, log_start_datetime, sensitivity=1, bckgrnd=0, data_freq=10, skip_start=0
    ...
    for file in filt_file_list:
        ...
        plt.plot(binary_data_dict['ref_counts_lin'], color='orange', alpha=0.7)
...
def load_hk(path, so2_cylinder_conc):
    ...
    df = df[
        ["Task", "ZA_SB_MFC_Read", "Cal_SO2_MFC_Read", "Cal_ZA_MFC_Read", "Time_s", "Cell_Flow", "Signal_LIF_Counts", "SO2_ppt", "Inlet_ZA_MFC_Read", "SO2_Cal_MFC_Set_P", "Cal_SB_Vl", "Cell_Pressure", "Thermistor_5", "T_board_NLO", "Laser_Power_PT_0"]
    ]
...
def identify_cal_amb_groups(
    ...
    cal_timerange_mod = cal_timerange[19:]
    #cal_timerange_mod = pd.concat(cal_timerange_mod[:])
    amb_timerange_mod = amb_timerange[2:54]
...
def calculate_amb_cal_points(hk, counts, cal_times, cyl_conc=5030, cal_remove_start=0, cal_remove_end=0, amb_remove_start=0, amb_remove_end=0):
    ...
    for i, row in enumerate(cal_times[1:].itertuples()):
        mr = (
...
def calculate_za_cal_points(hk, counts, cal_times, cyl_conc=5030, remove_start=0, remove_end=0):
    ...
    for i, row in enumerate(cal_times[26:].itertuples()):
        mr = (
