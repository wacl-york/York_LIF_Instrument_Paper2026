...
def load_hk(path, so2_cylinder_conc):
    ...
    df = df[
        ["Task", "ZA_SB_MFC_Read", "Cal_SO2_MFC_Read", "Time_s", "Cell_Flow", 
         "Signal_LIF_Counts", "SO2_ppt", "Inlet_ZA_MFC_Read", "SO2_Cal_MFC_Set_P", 
         "Cal_SB_Vl", "Cell_Pressure", "Thermistor_5", "T_board_NLO", 
         "Laser_Power_PT_0", "Signal_Dk_Counts", "Ref_Dk_Counts"]
    ]
...
def identify_cal_amb_groups(
    ...
    amb_timerange_mod = amb_timerange[5:]
...
def identify_cal_za_groups(
    ...
    za_cal_timerange_mod = za_cal_timerange[13:]
    
    for i, row in enumerate (za_cal_timerange_mod.itertuples()):
